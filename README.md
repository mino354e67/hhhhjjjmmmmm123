...
- 到点后，在 `MIN_GB`–`MAX_GB` 之间随机选一个目标字节数（默认 10–30 GB）
- 启动时对 `urls.txt` 里的每个 URL 做 `HEAD` 预检，自动剔除不可用 / 过小的文件
- 循环调用 `curl -o /dev/null --limit-rate 12500k ...` 直到累计下载达标
...
```
配置: 窗口 02:00-06:00, 每日 10.0-30.0 GB, 限速 12500k
...
| `MIN_GB` | `10` | 每次下载目标流量下限（GB） |
| `MAX_GB` | `30` | 每次下载目标流量上限（GB） |
...
```bash
# 跑一次完整的目标下载（按 MIN_GB~MAX_GB 随机）
docker compose exec speedtest-scheduler python /app/scheduler.py --once

# 只下载 100 MB 用来冒烟测试
docker compose exec speedtest-scheduler python /app/scheduler.py --once --target-mb 100
```
...
**Q: 想一天多跑几次？**
A: 目前是「每天窗口内随机一次」。如果想要「每天 2–3 次」，可以把 `WINDOW_END - WINDOW_START` 拆成几个小窗口，或改 `main_loop` 的逻辑（简单改成下载完再 `sleep` 一个随机 `60–120` 分钟直到窗口结束）。

**Q: 带宽比 100 Mbps 还小？**
A: 调低 `RATE_LIMIT` 和 `MAX_GB`。例如 50 Mbps + 5–10 GB：`RATE_LIMIT=6250k`、`MIN_GB=5`、`MAX_GB=10`。
